...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Function to add new content
        function addContentSs() {
            const newDiv = document.createElement('div');
            newDiv.classList.add('d-flex', 'justify-content-between', 'mt-1');
            newDiv.innerHTML = `
                <input type="text" class="form-control mx-1" name="screen_short_link[]" placeholder="Add Image link" value="">
                <a class="btn btn-danger btn-sm mx-1 text-white" id="closeSs"><i class="fa-solid fa-x"></i></a>
            `;

            // Append new div to ssContainer
            ssContainer.appendChild(newDiv);

            // Add event listener to close button
            const closeButtonSs = newDiv.querySelector('#closeSs');
            closeButtonSs.addEventListener('click', function() {
                ssContainer.removeChild(newDiv);
            });
        }

        // Event listener for Add button
        const addButtonSs = document.getElementById('addSs');
        addButtonSs.addEventListener('click', addContentSs);
    });
</script>
